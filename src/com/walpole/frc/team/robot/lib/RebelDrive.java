...
public class RebelDrive extends RobotDrive {
	...
	/*
	 * @Override public void arcadeDrive(double driveSpeed, double turnSpeed) {
	 * if (Speed.Slow.equals(Robot.drive.getCurrSpeed())) {
	 * arcadeDrive(driveSpeed * SLOW, turnSpeed, true); } else {
	 * arcadeDrive(driveSpeed * STRAIGHT_THROTTLE,
	 * lowSpeedNic.calculate(turnSpeed * TURN_THROTTLE), true);
	 * 
	 * } }
	 */
	...
	public void arcadeTankDrive(double driveLeftSpeed, double driveRightSpeed, double turnSpeed) {
		...
		// Use the average of the two drive speeds to find the direction of the
		// robot
		if (driveLeftSpeed > 0) {
			moveValue = 1;
		} else {
